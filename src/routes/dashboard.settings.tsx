  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Gerencie seu endereço público e sua sessão.</p>
      </div>

      <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        <h2 className="text-lg font-semibold">Endereço da Bio</h2>
        <p className="mt-1 text-xs text-muted-foreground">Ao mudar o username, o endereço público anterior deixa de apontar para sua página.</p>
        <div className="mt-5 space-y-2">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="flex flex-1 items-center rounded-xl border border-input bg-background px-3">
              ...
              <Input id="settings-username" value={username} onChange={(event) => setUsername(normalizeUsername(event.target.value))} className="border-0 bg-transparent px-1 shadow-none focus-visible:ring-0" />
            </div>
            <Button onClick={saveUsername} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? "Salvando..." : "Salvar"}</Button>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">O e-mail é gerenciado pelo sistema de autenticação.</p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        ...
        <Button variant="outline" className="mt-4" onClick={signOut}><LogOut className="mr-2 h-4 w-4" />Sair da conta</Button>
      </section>
    </div>
  );
}
